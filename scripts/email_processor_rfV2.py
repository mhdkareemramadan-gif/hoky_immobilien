"""
Email Processor — RandomForest V2 Model
Reads unread Gmail messages, uses Gemini to extract property features,
runs rf_v2_estate_model.joblib (bundle: model + TargetEncoder) to predict price,
and sends a reply. Prediction is on log scale — reversed with expm1.
"""
...
MODEL_PATH   = PROJECT_ROOT / "model" / "rf_v2_estate_model.joblib"
...
# --- Load bundle ---
print(f"Loading RFV2 bundle from {MODEL_PATH}...")
...
BOOL_MAP = {
    'y': True, 'yes': True, 'j': True, True: True, 1: True, '1': True,
    'n': False, 'no': False, False: False, 0: False, '0': False
}
...
def engineer_features(features: dict) -> pd.DataFrame:
    ...
    row = {
        "obj_livingSpace":    safe_float(features.get("obj_livingSpace"),    120.0),
        "obj_yearConstructed": safe_float(features.get("obj_yearConstructed"), 1973.0),
        "obj_noRooms":        safe_float(features.get("obj_noRooms"),         4.0),
        ...
        "obj_cellar":         BOOL_MAP.get(features.get("obj_cellar", "n"), False),
    }
...
Our Random Forest model (RFV2) has evaluated the property:
...
Best regards,
Automated Property Evaluator — RFV2
"""
            send_email(em["from"], f"Price Estimate (RFV2): {em['subject']}", reply)
