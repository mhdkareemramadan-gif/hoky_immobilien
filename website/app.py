...
def engineer_web_features(raw_data: dict) -> pd.DataFrame:
    ...
    # --- Engineered features (mirrors XGboost_model.py exactly) ---
    ...
    # --- Assemble row in exact training column order ---
    row = {
...
def predict():
    try:
        ...
        # Model was trained on log1p(price) — reverse with expm1
        prediction_log = model.predict(X)[0]
        prediction_eur = max(0.0, float(np.expm1(prediction_log)))

        return jsonify({"predicted_price": f"{prediction_eur:,.2f} EUR"})
